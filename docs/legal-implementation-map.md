...
| GT-CREDIT-029 | **deviates** (re-decided 2026-09-23: the store now gives the rates per edition, the engine holds one rate for every year) | `src/tax_law/treaty_withholding.py` `_TREATY_DIVIDEND_RATES`, FR/JP/CA/KR/NL/TW entries | `test_foreign_withholding_treaty_guard.py::test_b9_*` (FR at 25 % → 12,8; FR at 12,8 in full; TW 21 % → 10; JP 15,315 % → 15; a FR fund distribution not given the rate) | The BZSt column-C creditable dividend rates for FR, JP, CA, KR, NL, TW. Superseded position (store commit only): deviates, the engine carrying the US rate alone. Share dividends only — a fund distribution is outside the table's *Dividenden* and stays `FOREIGN_WHT_RATE_NOT_VERIFIED`. Measured 2026-09-23: every non-US withholding row in `Cash_Transactions-{2023..2025}.csv` is on a share. |
