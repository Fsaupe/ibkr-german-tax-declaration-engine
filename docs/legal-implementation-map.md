...
| GT-CREDIT-029 | **deviates** (added 2026-09-23; the engine carries the US rate only) | — | — | The BZSt column-C creditable dividend rates for FR, JP, CA, KR, NL, TW. The engine still keeps the withheld amount on Zeile 41 for every one of these states (`FOREIGN_WHT_RATE_NOT_VERIFIED`). Incidence at GT-CREDIT-026: 14 rows above 15 % + 1 ct in VZ 2023–2025, and France's creditable rate is 12,8, below that threshold. Closed by the `fix-func(engine)` that follows in the issue #78 PR (table rows, share dividends only — a fund distribution is outside the table's *Dividenden*). |
