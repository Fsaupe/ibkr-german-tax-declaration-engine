...
| GT-ESTG20-063 | **deviates** (store re-audit 2026-09-20; follow-up `fix-func(parsers)` in this series) | `src/parsers/grants_parser.py` — classifies a grant row by a substring of `ActivityDescription`; `src/engine/event_processors/stock_award_processor.py` — records the undeclared receipt | `tests/test_stock_award_scenarios.py::test_an_award_in_the_tax_year_reports_the_receipt_it_does_not_declare` | **Scope: Interactive Brokers' "Refer-A-Friend" share award only.** The store establishes § 22 Nr. 3 for that programme by applying the Veranlassung test to its terms; it is no longer a filing election (Q19 retired). **Deviation 1:** the substring match admits any row containing *Stock Award Grant*, so a row of another programme — in particular a purchase-conditioned one, BMF Rz. 129b ¶2, which is a cost reduction and no income — would be processed as this one. The follow-up replaces it with the three exact strings the supported programme writes. **Deviation 2, standing:** the receipt is not declared, because the reporting layer has no Anlage SO *Leistungen* category and `reference/tax-forms/anlage-so-zeilen.md` holds no Zeilen for it — issue #76. The run discloses amount, year and destination instead. **Assumption, stated and not guarded (owner's decision 2026-09-20: document only, no input, no warning):** nothing in the export names the programme. The engine supports exactly this one and treats rows carrying its three activity strings as belonging to it; a different programme writing identical strings cannot be told apart. README and `input_data_spec.md` say the same. |
| GT-ESTG20-064 | **implements** | `src/parsers/domain_event_factory.py` (`create_events_from_grants` — an award is dated on `AwardDate`, a vesting on `VestingDate`, a return on `ReportDate`); `src/processing/enrichment.py` (the award row's `Price` converted at the ECB rate of the award date); `src/engine/fifo_manager.py` (`add_lot_for_stock_award`, used by the historical replay and by `src/engine/event_processors/stock_award_processor.py` alike; a vesting is inert) | `tests/test_stock_award_lots.py::test_award_creates_a_lot_on_the_day_the_shares_arrived`, `::test_the_award_lot_is_final_and_no_vesting_operation_exists`; `tests/test_stock_award_scenarios.py::test_a_grant_inside_the_tax_year_creates_its_lot`, `::test_a_vesting_inside_the_tax_year_does_not_move_the_cost`, `::test_an_award_is_dated_on_its_award_date_not_the_broker_s_report_date` | **Zufluss on the booking day, value of that day** — the store's application of BMF 01.06.2024 Rn. 25-26 and the BFH line to the quoted Refer-A-Friend terms (dividends and vote from the award; price risk and margin use with the recipient; a contractual Sperrfrist; a resolutive reclaim). Q17 is retired: this is an application, not an election; the owner confirmed on 2026-09-20 that the awards came under that programme and agreed the application. **Correction 2026-09-20:** this row used to say Leitsatz 2 of VI R 37/09 *does not arise, on the law*, because the restriction is contractual. VI R 37/09 Rn. 20 says the source of a restriction is irrelevant and its effect decisive, so that sentence was wrong, and it was written before the terms had been read. **The dependency the store states, carried here so it is not hidden:** the terms call a premature disposal *void*. If under New York law such a transfer were ineffective and not merely a breach, Zufluss would move to `VestingDate`, the lot would take the vesting row's `Price` at that day's ECB rate, and a return before vesting would have no tax effect. Nothing located establishes that; the engine does not implement that path, and adopting it would be a `fix-func(engine)`. **Input contract:** acquisition date ← `AwardDate`; unit value ← the award row's `Price` in `CurrencyPrimary`; EUR ← ECB rate of `AwardDate`. An unparseable date, a zero quantity or a failed conversion stops the run (collected in `create_events_from_grants`; `add_lot_for_stock_award` refuses a lot without a EUR cost). **Residual, not resolvable from the files:** no column says which day `Price` was struck on; on every award row of the contributor's files `ReportDate` equals `AwardDate`, so the question does not arise there (measured 2026-09-20 by comparing the two columns row by row). |
| GT-ESTG20-065 | **implements** | `src/engine/fifo_manager.py` — the awarded lot's `unit_cost_basis_eur` is the award-day value and is what a later disposal is measured against | `tests/test_stock_award_lots.py::test_the_whole_sequence_leaves_the_broker_s_quantity_and_the_awarded_cost`; `tests/test_stock_award_scenarios.py::test_a_sub_freigrenze_award_still_carries_full_basis` | The value at Zufluss is the Anschaffungskosten: acquisition for consideration (§ 20 Abs. 4 Satz 1, § 255 Abs. 1 Satz 1 HGB), confirmed without regard to taxation of the receipt by BMF 06.03.2025 Rn. 75 and BMF 14.05.2025 Rz. 87 — both analogies, labelled so in the store. **Q16 is retired; this is no longer a filing position.** The earlier *choice under uncertainty* wording and the *what was taxed becomes the basis* rationale are withdrawn. The amount depends on the Zufluss day, so it shares GT-ESTG20-064's stated dependency. |
| GT-ESTG20-066 | **deviates** (follow-up `fix-nonfunc(engine)` in this series removes the warning) | `src/utils/sorting_utils.py` — a `StockAwardEvent` sorts in the lot-delivering band, ahead of the day's trades; `src/engine/fifo_manager.py` (`reverse_stock_award_lot`) — a return removes units of its own award's lot at that lot's cost and refuses to remove more than it holds; `src/engine/calculation_engine.py` — records a `STOCK_AWARD_REVERSAL_ORDER_ASSUMED` WARNING on a same-day return and sale | `tests/test_stock_award_scenarios.py::test_a_same_day_reversal_and_sale_apply_reversal_first_and_warn` | No rule of law orders a same-day return against a disposal, and that is not a disputed reading (Q18 retired as never having been a legal question). The law's requirement is that each event has its own effect, which the two ledger operations give it. **Deviation:** the WARNING presents a scheduling convention as something the reader must assess; it discloses no unsupported figure and is to be removed. The engineering fact that the two orders cannot yield two figures — a return takes its own lot at its own cost, a sale consumes FIFO, so both complete to the same result or the return aborts for want of units — belongs with the code and its test, not in the store. |
| GT-ESTG20-067 | **deviates** (the run calls a settled point unsettled; follow-up `fix-func(engine)` in this series) | `src/engine/fifo_manager.py` (`reverse_stock_award_lot` — removes the returned units at the lot's own cost, no `RealizedGainLoss`); `src/engine/event_processors/stock_award_processor.py` — records `STOCK_AWARD_RETURN_TREATMENT_UNSETTLED` for a return inside the processed year | `tests/test_stock_award_scenarios.py::test_an_in_year_reversal_reports_the_unsettled_return_treatment` | **Q20 is retired.** A return after Zufluss is a negative Einnahme of the year of return (EStH H 22.8; BFH IX R 26/14 Rn. 20 — same Rechtsverhaeltnis, back to the payer, both met by the programme's reclaim), not a retroactive correction (BFH VI R 6/18 Rn. 37-40), in the amount originally brought to account (BFH VI R 17/08 Ls. 2). **The ledger side implements this:** the units leave at their own Anschaffungskosten and nothing is realised. **Deviation:** the warning text and its test still tell the reader the treatment is unsettled and give no amount. The follow-up states the amount (the returned units × the award's unit value), the year (of the return) and the destination (Anlage SO), undeclared for the same reason as the receipt (issue #76). Because the engine applies booking-day Zufluss (GT-ESTG20-064), every return follows Zufluss; the store's *no receipt, no repayment* boundary is therefore not reached. **Input contract:** the award a return belongs to ← the return row's `AwardDate` with its account; units ← `Quantity`; amount ← the lot's own unit cost, never the return row's `Price`. A return naming an award the ledger does not hold, or more units than it holds, stops the run. |
