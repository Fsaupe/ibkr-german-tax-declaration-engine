...
| GT-ESTG20-010 | **not reached**, re-decided 2026-08-09 | — | — | Besondere Entgelte und Vorteile. Position unchanged, ground replaced: superseded note *"No broker input maps to one"*. Satz 1 has **two** alternatives and *an deren Stelle* is where a substitute payment lands if the § 39 AO attribution went to the borrower — [GT-INVSTG-059] branch B. Not reached because the engine takes branch A unconditionally, not because no input could produce one. **A second candidate, 2026-09-21 (Q21):** a positive net commission on a trade occurs in the broker's exports, and reading B of Q21 routes it to Satz 2 of this claim (*"sonstige Aufwendungen ... erstattet"*); see GT-ESTG20-069. **Reading A was chosen by the taxpayer**, so the credit is part of the trade and nothing reaches Satz 2. If Q21 is ever resolved as B, this is the row that flips — together with GT-ESTG20-069. |
...
| GT-ESTG20-069 | **choice under uncertainty — reading A of Q21**, chosen by the taxpayer 2026-09-21 | `src/processing/enrichment.py` folds the commission into `net_proceeds_or_cost_basis_eur` with its sign: a credit lowers a buy's cost and raises a sale's proceeds. The currency side already followed the sign (`trade_processor._process_commission_currency_impact`, the historical currency replay): a credit creates a currency lot | `test_commission_rebate.py`; `test_console_trade_report.py` (the diagnostic's local net) | **What was put to the taxpayer:** both readings with the authority behind each, that the amount is taxable capital income under either, and that B is the more defensive in timing and pot while A is the better supported. Asked which reading over-declares, the answer given was B; A was then chosen. **Why A can be computed and B cannot:** the export's commission is always a net of the broker's charge and the venue's rebate, so B's split — rebate as income, charge as cost — is not in the data, on these rows or on the ordinary ones where a smaller rebate sits inside a negative commission. **Purchase side:** Tier 1 (§ 255 Abs. 1 S. 3 HGB) with Tier 4 (BFH IX R 46/03) and the administration's nearest decided case at Tier 2 (BMF Rz. 129b third paragraph: a premium tied to buying securities reduces their cost). **Disposal side:** by the same logic only; no sentence says it. Not a lean-to-the-taxpayer case: no dispute is documented, so that rule is not invoked. **Probed 2026-09-21, bytecode caching off, 5 of 5 caught:** each line flattened back, the credit ignored on a buy, and the foreign amount folded in place of its EUR value on each line. |
