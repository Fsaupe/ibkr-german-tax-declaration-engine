...
| GT-ESTG20-074 | implementation in progress | `src/engine/short_sale_disclosure.py`; `src/reporting/short_sales.py`; console/PDF reporters | `tests/test_short_sale_disclosure.py` | Complete account/lot schedules at year-end and prior-year covers, explicit zero contribution, selected reasoning and contrary authority. Applies to actual securities shorts, not option/future/CFD or currency short exposure. The annex's inventory date is explicit; subsequent facts known when filing must be supplied too. No certification of a particular taxpayer's complete filed return. |
