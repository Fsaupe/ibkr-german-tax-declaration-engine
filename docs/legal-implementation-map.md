...
| GT-ESTG20-063 | **deviates** (the receipt is disclosed, not declared — issue #76) | `src/parsers/grants_parser.py` — admits only the three whole `ActivityDescription` strings the supported programme writes (`KNOWN_ACTIVITIES`) and stops the run on any other; `src/parsers/domain_event_factory.py` (`create_events_from_grants`) dispatches on the same three; `src/engine/event_processors/stock_award_processor.py` — records the undeclared receipt | `tests/test_stock_award_scenarios.py::test_a_row_of_another_programme_stops_the_run`, `::test_an_unclassified_activity_kind_stops_the_run`, `::test_a_vesting_in_the_tax_year_reports_the_receipt_it_does_not_declare` | **Scope: Interactive Brokers' "Refer-A-Friend" share award only.** The store establishes § 22 Nr. 3 for that programme by applying the Veranlassung test to its terms; it is no longer a filing election (Q19 retired). A row of any other programme — in particular a purchase-conditioned one, BMF Rz. 129b ¶2, which is a cost reduction and no income — is refused, not processed as this one. **The deviation:** the receipt is not declared, because the reporting layer has no Anlage SO *Leistungen* category and `reference/tax-forms/anlage-so-zeilen.md` holds no Zeilen for it — issue #76. The run discloses amount, year and destination instead, on the console and in the PDF — for the vesting, which is where the receipt falls under the reading taken at GT-ESTG20-064. **Programme identity is confirmed, not assumed:** nothing in the export names the programme, and a different programme writing the same three activity strings cannot be told apart from the text. `ParsingOrchestrator` therefore stops a run whose Grants files hold rows unless `config.STOCK_AWARD_PROGRAMME` is `"IBKR_REFER_A_FRIEND"` — the user's statement, made once and kept in config (`tests/test_stock_award_scenarios.py::TestTheProgrammeIsConfirmedNotAssumed`). An earlier *document only, no input* decision by the contributor was not accepted in review and is withdrawn. |
| GT-ESTG20-064 | **choice under uncertainty** (Q17, Reading A) | `src/parsers/domain_event_factory.py` (`create_events_from_grants` — an award is dated on `AwardDate`, a vesting on `VestingDate`, a return on `ReportDate`; a vesting row without a positive `Price` stops the run); `src/processing/enrichment.py` (the vesting row's `Price` converted at the ECB rate of the vesting date); `src/engine/fifo_manager.py` (`add_lot_for_stock_award` books a `zufluss_pending` lot without cost or acquisition date; `vest_stock_award_lot` gives it both; `consume_long_lots_for_sale` skips unvested lots and refuses a disposal that would reach them; `_refuse_with_unvested_award` stops a cash merger, a stock merger and a capital repayment), used by the historical replay and by `src/engine/event_processors/stock_award_processor.py` alike | `tests/test_stock_award_lots.py::test_award_books_the_units_on_the_day_they_arrived_without_a_cost`, `::test_vesting_gives_the_lot_the_vesting_day_and_the_vesting_value`, `::test_a_sale_never_consumes_unvested_units`, `::test_a_sale_reaching_into_unvested_units_stops_the_run`, `::test_an_event_that_reprices_every_lot_stops_the_run_while_units_are_unvested`; `tests/test_stock_award_scenarios.py::test_a_vesting_inside_the_tax_year_sets_the_date_and_the_cost`, `::test_a_sale_of_unvested_shares_stops_the_run`, `::test_a_vesting_is_dated_on_its_vesting_date_not_the_broker_s_report_date` | **Reading A of Q17: Zufluss on the day the transfer restriction lapses, value of that day.** Where the settled law ends is stated at the claim: BMF 01.06.2024 Rn. 25-26 and the BFH line fix the rule on both sides, and no German source at any tier says on which side this programme's *void* clause falls -- that turns on the effect of the clause under the foreign law governing the holding, a fact nobody has established. **Decided by the maintainer (uebber), 2026-09-20:** the effect of one customer's contract is a matter of that individual legal relationship, not something this engine can find; Reading A is taken, and recorded as **defensible (vertretbar) and not negligent**. Why it is defensible: the terms do not merely forbid a premature disposal, they declare it *void*, and the broker, who is also the custodian, refuses to process one -- so the holder has no route to an effective disposal, which is the case of VI R 37/09 Rn. 15 and 20 and of BMF Rn. 26 first sentence; dividends and votes alone do not establish a *rechtlich gesicherte Inhaberschaft* (Rn. 17); and it contradicts no Tier 1 or Tier 2 source. Why it is not negligent: the question was researched, both readings and their authority are written into the store as Q17, and the choice is stated here, in the README and in the run's output instead of being made in silence. The contributor, whose declaration it is, follows the same reading. An earlier version of this row recorded Reading B as the maintainer's choice; that rested on a misunderstanding between contributor and maintainer and was corrected the same day. **What Reading B would change**, so the choice can be reversed if the fact is ever established: Zufluss on `AwardDate`, the lot final from the award at the award row's `Price` and that day's ECB rate, a vesting inert, and a return a negative Einnahme of its year (GT-ESTG20-067). **Input contract:** units booked ← award row `Quantity` on `AwardDate`; acquisition date ← `VestingDate`; unit value ← the vesting row's `Price` in `CurrencyPrimary`; EUR ← ECB rate of `VestingDate`; the vesting must name exactly the units the award still holds after returns. An unparseable date, a zero quantity, a vesting `Price` that is not positive, a failed conversion, a vesting without its award, a quantity disagreement, or a disposal reaching unvested units stops the run. **Assumption the code relies on, at zero incidence in the contributor's files:** a vesting row carries the `ClientAccountID` of the ledger that holds the award's lot; an award moved to another account before it vests, and reported under a different account, stops the run rather than vest another lot. **Residual, not resolvable from the files:** no column says which day the vesting `Price` was struck on; `ReportDate` differs from `VestingDate` on 2 of the 3 vesting rows of the contributor's files (counted 2026-09-20), so the price is the broker's figure for the vesting, used as given, and not independently checked against that day's close. |
| GT-ESTG20-065 | **implements** | `src/engine/fifo_manager.py` (`vest_stock_award_lot`) — the vested lot's `unit_cost_basis_eur` is the vesting-day value and is what a later disposal is measured against | `tests/test_stock_award_lots.py::test_the_whole_sequence_leaves_the_broker_s_quantity_and_the_vested_cost`; `tests/test_stock_award_scenarios.py::test_a_sub_freigrenze_award_still_carries_full_basis`, `::test_a_currency_vesting_is_converted_at_the_event_date_not_left_foreign` | The value at Zufluss is the Anschaffungskosten: acquisition for consideration (§ 20 Abs. 4 Satz 1, § 255 Abs. 1 Satz 1 HGB), confirmed without regard to taxation of the receipt by BMF 06.03.2025 Rn. 75 and BMF 14.05.2025 Rz. 87 — both analogies, labelled so in the store. **Q16 is retired; this is no longer a filing position.** The amount depends on the Zufluss day, so it follows the reading taken at GT-ESTG20-064 (Q17, Reading A, maintainer's decision of 2026-09-20): the vesting row's `Price` at the vesting day's ECB rate. |
| GT-ESTG20-066 | **implements** | `src/engine/fifo_manager.py` — `reverse_stock_award_lot` takes unvested units of its own award's lot; `consume_long_lots_for_sale` is measured against acquired units only and never touches an unvested lot | `tests/test_stock_award_scenarios.py::TestASameDayReturnAndSale::test_the_sale_is_measured_against_the_bought_shares` — current year and historical replay, both input row orders | No rule of law orders a same-day return against a disposal, and that is no election (Q18 retired as never a legal question). What the law requires is that each event has its own effect. Under the reading taken at GT-ESTG20-064 (Q17, Reading A) the two cannot compete for a unit at all: a return takes shares that have not vested, and a disposal consumes FIFO (GT-ESTG20-012) among acquired shares only. The result therefore cannot depend on how the input orders them, which the test asserts under both row orders. The `STOCK_AWARD_REVERSAL_ORDER_ASSUMED` warning stays removed. **Known blind spot, unchanged:** deleting the `StockAwardEvent` branch of `get_event_sort_key` leaves the award tests green (re-measured 2026-09-20: 49 passed) — the event carries no broker transaction id, and an empty id sorts ahead of every trade's in the shared key tail. CLAUDE.md, *Where the suite is blind*. |
| GT-ESTG20-067 | **not reached** (the negative Einnahme); the Boundary **implements** | `src/engine/fifo_manager.py` (`reverse_stock_award_lot` — takes unvested units out of the award's lot, produces no `RealizedGainLoss`, returns nothing to state an Einnahme from, and **stops the run** on a return naming an award that has already vested) | `tests/test_stock_award_lots.py::test_a_return_before_vesting_takes_units_and_is_no_tax_event`, `::test_a_return_after_vesting_stops_the_run`, `::test_reversing_more_than_was_awarded_stops_the_run`; `tests/test_stock_award_scenarios.py::test_an_award_and_a_return_before_vesting_report_nothing`, `::TestTheAnlageSoAmountsReachTheRunsOutput::test_an_award_and_a_return_in_the_year_state_nothing`, `::TestAwardedSharesReachTheLedger::test_a_return_before_vesting_is_not_a_disposal` | The claim's rule — a return after Zufluss is a negative Einnahme of the year of the return at the original value (EStH H 22.8; BFH IX R 26/14 Rn. 20; VI R 6/18 Rn. 37-40; VI R 17/08 Ls. 2) — is **not reached**: under the reading taken at GT-ESTG20-064 (Q17, Reading A) Zufluss falls on the vesting, and the supported programme reclaims shares only before it. What applies is the claim's **Boundary**, which the engine implements: shares reclaimed before Zufluss never accrued, so there is neither a receipt nor a negative Einnahme, and nothing is disclosed for them. A return row dated after its award's vesting is outside what the terms produce; the run stops rather than apply either treatment. Q20 is retired. |
