...
| GT-CREDIT-026 | **deviates** (store landed; guard is the follow-up in this PR) | — (engine sums the withheld amount into `ANLAGE_KAP_FOREIGN_TAX_PAID` uncapped) | — (Group B guard not yet built) | The Ermäßigungsanspruch reduction to the treaty rate is not yet applied: a row withheld above the treaty rate would over-declare Zeile 41 by the excess. **Incidence measured 0 of 24 US stock/ETF withholding rows, VZ 2023–2025** (issue #78 research); the exposure is a future lapsed W-8BEN (every US row at 30 %). The Satz 1/Satz 3 ceilings stay the Finanzamt's (GT-CREDIT-005/006); this row is the distinct treaty-rate cap. Follow-up: the cap-and-report guard (Group B) flips this to `implements`. |
| GT-CREDIT-027 | **deviates** (store landed; guard is the follow-up in this PR) | — | — | The US treaty dividend rate (15 %) is not yet expressed in code; the guard's US-only treaty-rate table supplies it. Other source states remain unsourced and are reported as rate-not-verified, not defaulted. |
| GT-CREDIT-028 | **implements (income, branch A); credit figure invariant** | `src/parsers/domain_event_factory.py` (branch A income character, per GT-INVSTG-059); credit route via GT-CREDIT-004/026 | — (the guard tests at Group B assert the 15 % credit) | **Not an open question:** the creditable amount is 15 % under both treaty readings (Art. 10 on branch A, Art. 21 on branch B) because § 32d Abs. 5 Satz 1 reduces the credit only by an Ermäßigungsanspruch the source state grants, and the US grants none below 15 % (GT-CREDIT-026). Both readings and authorities are recorded in `reference/tax-law/dba-usa.md`; no `open-legal-questions.md` entry, since no declared figure depends on the choice. Residual risk stated in the store. |
