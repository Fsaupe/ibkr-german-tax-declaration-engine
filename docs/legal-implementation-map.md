...
| GT-ESTG20-063 | **deviates** | `src/engine/event_processors/stock_award_processor.py` — records the undeclared receipt as a data gap; nothing declares it | `tests/test_stock_award_scenarios.py::test_an_award_in_the_tax_year_reports_the_receipt_it_does_not_declare`, and `::test_a_vesting_and_a_reversal_report_no_receipt` for the other two kinds | A benefit granted for placing capital is a Leistung under § 22 Nr. 3, not Kapitalertrag — the boundary the administration itself draws at BMF 14.05.2025 Rz. 129b (a premium for a capital move is § 22 Nr. 3, and only a premium conditioned on a securities purchase is the ¶2 cost-reduction case, which this benefit is not), and the same three-step result the store reached for the Wertpapierdarlehen fee at GT-ESTG20-049. Nr. 7 fails on its charging element — the receipt is not an *Entgelt für die Überlassung des Kapitalvermögens zur Nutzung* ([GT-ESTG20-003]) — not on its gate. **The engine does not declare the receipt**, because the reporting layer has no Anlage SO *Einnahmen aus Leistungen* category — only `ANLAGE_SO_Z54_NET_GV`, for § 23 — and `reference/tax-forms/anlage-so-zeilen.md` holds no Zeilen for the Leistungen half of the form. That is the same gap as **issue #76**, and it is that issue's to close for both payment kinds at once. The receipt's year of Zufluss is the award booking day ([GT-ESTG20-064]), so the gap is recorded only for an award (`STOCK_AWARD_GRANTED`) dated inside the processed year; an award dated earlier is applied by the historical replay, which records no receipt. Measured 2026-09-02 across `Grants-2022.csv`…`Grants-2025.csv`: award events fall on `AwardDate` — 2 in 2022, 1 in 2023, none after — so the gap fires in **VZ 2023** (awards dated 2022 belong to VZ 2022, which is never processed). |
...
| GT-ESTG20-066 | **implements** (choice under uncertainty) | `src/utils/sorting_utils.py` — a `StockAwardEvent` (including a reversal) takes the lot-delivering band, so a reversal sorts before a same-day disposal (Reading A); `src/engine/calculation_engine.py` — a same-day award reversal and disposal of one security record a `STOCK_AWARD_REVERSAL_ORDER_ASSUMED` data gap (WARNING) | `tests/test_stock_award_scenarios.py::test_a_same_day_reversal_and_sale_apply_reversal_first_and_warn` | Q18: no Tier 1 or Tier 2 source fixes the intra-day order of a same-day award reversal and a disposal of the same security. FIFO ([GT-ESTG20-012]) orders a disposal's own lots, not a disposal against a non-disposal event, and the award report carries no intra-day id. **Reading A (reversal-first) chosen by the taxpayer (Fsaupe) on 2026-09-19** as a filing position, with a WARNING emitted on the collision so the assumption is visible and revisable. **What it moves:** which lot's Anschaffungskosten a same-day disposal uses — only where the awarded lot is within the disposal's FIFO reach; the direction depends on that lot's basis. **Grey-area conditions (all met):** against neither Tier 1 nor Tier 2 (both silent on the sequence); BMF has not spoken; disputed not constructed — the law genuinely leaves the tax order of a same-day disposal and a non-disposal event unfixed (§ 20 Abs. 4 Satz 7 orders only a disposal's own lots), which is the question of law; the export's silence on the intra-day sequence is a separate, evidential gap and is not what makes this a grey area; no invented input (a position, not a value); asked and chosen by the taxpayer 2026-09-19. **Incidence zero** — the one reversal is VZ 2022 with no same-day disposal; recorded because the event sort would otherwise settle the order silently. |
