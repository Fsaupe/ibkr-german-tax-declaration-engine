...
| GT-ESTG20-062 | not reached | — | — | § 22 Nr. 3 Satz 1's subsidiarity clause and Satz 2's 256-Euro Freigrenze, with Satz 3 and Satz 4 on losses. No engine behaviour turns on it directly; it is the ground of GT-ESTG20-063, and it is what makes the receipt side of an award a matter for Anlage SO rather than Anlage KAP. The Freigrenze itself is out of scope, as GT-ESTG23-009. The undeclared-receipt warning in `src/engine/event_processors/stock_award_processor.py` names it — with GT-ESTG23-009 for the shared rationale — as the § 22 Nr. 3 Freigrenze it declines to apply; an explanatory citation, no behaviour turns on it, so the row stays *not reached*. |
| GT-ESTG20-063 | **deviates** (§ 22 Nr. 3 is a choice under uncertainty, Q19) | `src/engine/event_processors/stock_award_processor.py` — records the undeclared receipt as a data gap; nothing declares it | `tests/test_stock_award_scenarios.py::test_an_award_in_the_tax_year_reports_the_receipt_it_does_not_declare`, and `::test_a_vesting_and_a_reversal_report_no_receipt` for the other two kinds | A benefit granted for placing capital is treated as a **§ 22 Nr. 3 Leistung — a taxpayer filing choice in a grey area (Q19), made 2026-09-20, not a settled classification**. Rz. 129b ¶1 (a depot-transfer premium is § 22 Nr. 3) and the Startguthaben practice support Nr. 3 by analogy; the § 20 Abs. 1 Nr. 7 reading — the keep-the-deposit condition as an *Entgelt für die Überlassung des Kapitalvermögens zur Nutzung*, BFH VIII R 18/20 — is the live alternative. **Grey-area conditions:** no Tier 1/2 source is on the fact pattern (Rz. 129b addresses only a depot transfer and a securities-conditioned premium, not a cash deposit left in place); BMF has not spoken on it; the ambiguity is genuine (two authorities pull opposite ways), not constructed; no invented input; the taxpayer chose Nr. 3. **No declared figure turns on the choice today** — the receipt is not declared (below) and the later disposal basis is the value at Zufluss under either reading — but it fixes which Anlage the receipt belongs on and whether the Freigrenze and Q16 apply. **The engine does not declare the receipt**, because the reporting layer has no Anlage SO *Einnahmen aus Leistungen* category — only `ANLAGE_SO_Z54_NET_GV`, for § 23 — and `reference/tax-forms/anlage-so-zeilen.md` holds no Zeilen for the Leistungen half of the form. That is the same gap as **issue #76**, and it is that issue's to close for both payment kinds at once. The receipt's year of Zufluss is the award booking day ([GT-ESTG20-064]), so the gap is recorded only for an award (`STOCK_AWARD_GRANTED`) dated inside the processed year; an award dated earlier is applied by the historical replay, which records no receipt. Measured 2026-09-02 across `Grants-2022.csv`…`Grants-2025.csv`: award events fall on `AwardDate` — 2 in 2022, 1 in 2023, none after — so the gap fires in **VZ 2023** (awards dated 2022 belong to VZ 2022, which is never processed). |
| GT-ESTG20-064 | **implements** | `src/parsers/domain_event_factory.py` (`create_events_from_grants` — an award is dated on `AwardDate`, a vesting on `VestingDate`, a reversal on `ReportDate`); `src/processing/enrichment.py` (the award price converted at the ECB rate for the event's own date); `src/engine/fifo_manager.py` (`add_lot_for_stock_award` and `reverse_stock_award_lot`, applied by the historical replay; a vesting is inert and no restatement method exists); `src/engine/event_processors/stock_award_processor.py` (the same methods for an award dated inside the tax year) | `tests/test_stock_award_lots.py` — `test_award_creates_a_lot_on_the_day_the_shares_arrived`, `test_the_award_lot_is_final_and_no_vesting_operation_exists`; `tests/test_stock_award_scenarios.py` — `test_a_grant_inside_the_tax_year_creates_its_lot` (red without the current-year processor), `test_a_vesting_inside_the_tax_year_does_not_move_the_cost` | Valuation of a benefit in kind at the ueblicher Endpreis (§ 8 Abs. 2 Satz 1) on the day of Zufluss (§ 11 Abs. 1 Satz 1). **Zufluss is the booking.** BFH VI R 37/09 Rn. 12 holds that a Sperr- or Haltefrist does not prevent it and Leitsatz 2 (Rn. 15) that what does is a disposal being *rechtlich unmoeglich*, so the engine dates the acquisition on `AwardDate` and a later vesting has no ledger effect at all. **A grey area, and the taxpayer's to settle (Q17):** the alternative reading dates Zufluss at the lapse of the condition; the research run pointed to the booking and the taxpayer chose the booking on **2026-09-02**. The citation is Tier 4 and decided under § 19 EStG; § 11's Zufluss concept is what is borrowed, and that no located source carries it to a § 22 Nr. 3 benefit from a broker is recorded at Q17. **Leitsatz 2's case does not arise, on the law rather than a measurement:** it needs a disposal to be *rechtlich unmoeglich*, and a contractual clawback condition is not a legal impossibility — Rn. 12 holds a contractual Sperr- or Haltefrist does not postpone Zufluss even where it blocks a sale. So the booking-day Zufluss does not turn on establishing that these particular shares were freely disposable. **Measured on the real export (2026-09-02):** with the report read, the awarded shares reconcile against the broker snapshot at every checkpoint mark and no fallback lot is synthesised — VZ 2023 (1 mark), VZ 2024 (2), VZ 2025 (3), all kept; without the report the same run synthesises one fallback lot with an invented acquisition date (`REPLAY_MARK_UNCONFIRMED_START`, `{tax_year-1}-12-31`). **Residual uncertainty, recorded rather than resolved:** no column states which day `Price` was struck on, so where `ReportDate` and `VestingDate` differ the ueblicher Endpreis used may be either day's. It is the broker's measurement and is used as given; the alternative is to invent one from market data this engine does not hold. |
| GT-ESTG20-065 | **implements** (choice under uncertainty) | `src/engine/fifo_manager.py` — the awarded lot's `unit_cost_basis_eur` (the award-day price) is what a later disposal is measured against | `tests/test_stock_award_lots.py::test_the_whole_sequence_leaves_the_broker_s_quantity_and_the_awarded_cost`, which asserts the cost is the award-day cost while the quantity follows the awards and reversals; `tests/test_stock_award_scenarios.py::test_a_sub_freigrenze_award_still_carries_full_basis`, which pins Reading A where the year's Leistung stays under the Freigrenze | The value brought to tax on receipt is the Anschaffungskosten of the securities on a later disposal under § 20 Abs. 2 Satz 1 Nr. 1. This is the claim a later declared disposal figure turns on: without it the disposal has no basis at all. **Open question Q16** — whether a receipt the Freigrenze left untaxed still supplies Anschaffungskosten at full value — **is a choice the engine makes**, not a question it avoids: `add_lot_for_stock_award` assigns the full award-day value as basis unconditionally, which is **Reading A** (full value regardless of the Freigrenze). **Chosen by the taxpayer (Fsaupe) on 2026-09-20** as a filing position — the favourable reading, since a higher basis lowers a later gain, and lean-to-taxpayer applies. Grey-area conditions all met: no Tier 1/2 source settles the interaction (BMF Rz. 129b is silent on it), both readings with their authorities are in `reference/research/open-legal-questions.md` Q16, and the data are complete so this is a position, not an invented input. It differs from Reading B only in a year whose Leistungen stayed under the Freigrenze (GT-ESTG20-062, itself out of scope as GT-ESTG23-009); there, Reading A is the chosen position. |
| GT-ESTG20-066 | **implements** | `src/utils/sorting_utils.py` — a `StockAwardEvent` (including a reversal) takes the lot-delivering band, so a reversal is applied before a same-day disposal; `src/engine/calculation_engine.py` — a same-day award reversal and disposal of one security in one account record a `STOCK_AWARD_REVERSAL_ORDER_ASSUMED` data gap (WARNING) | `tests/test_stock_award_scenarios.py::test_a_same_day_reversal_and_sale_apply_reversal_first_and_warn` | Q18: no Tier 1 or Tier 2 source fixes the intra-day order of a same-day award reversal and a disposal of the same security — FIFO ([GT-ESTG20-012]) orders a disposal's own lots, not a disposal against a non-disposal event, and the award report carries no intra-day id. **But the order is figure-neutral by construction, so no figure turns on it and this is not a taxpayer choice.** A reversal removes its own `(account, award_date)` lot at that lot's own cost and refuses over-reversal; a disposal consumes FIFO. So a same-day reversal and disposal that both complete give the same figure in either order; a disposal-first order differs only by aborting on over-reversal, which is the correct outcome for inconsistent input. The engine applies the reversal first, which completes wherever the input is consistent. **Correction 2026-09-20:** an earlier revision recorded this as a grey-area "Reading A" the taxpayer chose as a favourable filing position; measuring the mechanism showed there is no second figure to choose, so the position is downgraded to a plain `implements` and the WARNING reworded to disclose an unusual same-day pair, not a figure in doubt. **Incidence zero** — the one reversal is VZ 2022 with no same-day disposal. |
| GT-ESTG20-067 | **deviates** (return treatment open, Q20) | `src/engine/event_processors/stock_award_processor.py` — an award reversal dated inside the processed year records a `STOCK_AWARD_RETURN_TREATMENT_UNSETTLED` data gap (WARNING); nothing declares a negative § 22 Nr. 3 income | `tests/test_stock_award_scenarios.py::test_an_in_year_reversal_reports_the_unsettled_return_treatment` | The § 22 Nr. 3 income consequence of returning a taxed benefit is unsettled (Q20): a negative Leistungseinnahme in the year of return, or a retroactive reduction of the receipt — only Tier 4/5 sources address it. The **disposal side is settled and correct**: the returned shares are dropped, so a later disposal is measured only against the shares retained at their own basis (§ 20 Abs. 2), which no reading disturbs. The **negative § 22 Nr. 3 income is not declared**, like the receipt itself (GT-ESTG20-063, issue #76) — there is no Anlage SO Leistungen line — so an in-year reversal records a WARNING naming Q20 so the taxpayer handles the Anlage SO consequence. **WARNING, not FAIL_FAST:** the declared KAP figures are safe; only the undeclared SO side is open. A pre-tax-year (historical) reversal is applied by the replay and records no receipt or return, so it does not warn. **Incidence zero** in processed years — the one reversal is VZ 2022. |
...
| GT-FX-009 | implements | `src/engine/calculation_engine.py` — `currency_fifo_ledgers` keyed `(account, asset_id)` with the real account, `currency_ledger_accounts` deciding which exist (opening cash balances, historical events, tax-year events; not the closing balances), `_ensure_currency_ledger_exists` per account, `_reconcile_currency_soy` against that account's own opening snapshot, and the per-account currency EoY check; the currency lookup in every event processor (`account_key(event.account_id)`) | `test_per_account_currency.py`, `test_group7_currency_fifo.py`, `test_data_gap_channel_guards.py`, `test_person_level_snapshot.py::TestACurrencyHeldInTwoAccounts` | Each account's balance is its own Kapitalforderung, so a disposal consumes what was paid into that account and the end-of-year check runs per account — being too high in one and too low in another cancels out in the person's total, which a person-level check passed. The person-level figures remain as the sum (`person_snapshot` over the cash rows in `soy/eoy_positions`), and no ledger is built from them. **What it rests on:** the cash report naming every account, the same thing every other per-account figure rests on. **Partly covered:** a currency held in an account the cash report omits gets a ledger only from its own events, its opening balance unknown rather than zero. Its **closing** balance is no longer skipped in silence: with no reported balance to check a non-zero ledger against, the run records `CURRENCY_EOY_UNRECONCILED` (WARNING) so the account and the uncertainty in the §20 gains computed from it reach the report — absent is not empty (`test_the_unreconciled_ledger_is_recorded_not_skipped`, and `test_currency_unreconciled_gaps_reach_the_pdf` for the PDF end). **The sub-0.01 threshold** in `_process_cash_balance_positions` no longer causes this for a supplied dust row: its **closing** is now recorded and compared, not dropped (F4), so only an account the cash report omits entirely reaches `CURRENCY_EOY_UNRECONCILED`. The threshold still does not **seed an opening** balance from such a row, so per account more openings are taken as unknown; the condition predates this change, which only makes it per account. **Still open:** the account's *opening* balance is taken as unknown rather than reconciled, and a total-level backstop or a lower threshold is not attempted here. |
