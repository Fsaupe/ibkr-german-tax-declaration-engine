...
| GT-ESTG20-064 | **choice under uncertainty** (Q17, Reading B) | `src/parsers/domain_event_factory.py` (`create_events_from_grants` — an award is dated on `AwardDate`, a vesting on `VestingDate`, a return on `ReportDate`); `src/processing/enrichment.py` (the award row's `Price` converted at the ECB rate of the award date); `src/engine/fifo_manager.py` (`add_lot_for_stock_award`, used by the historical replay and by `src/engine/event_processors/stock_award_processor.py` alike; a vesting is inert) | `tests/test_stock_award_lots.py::test_award_creates_a_lot_on_the_day_the_shares_arrived`, `::test_the_award_lot_is_final_and_no_vesting_operation_exists`; `tests/test_stock_award_scenarios.py::test_a_grant_inside_the_tax_year_creates_its_lot`, `::test_a_vesting_inside_the_tax_year_does_not_move_the_cost`, `::test_an_award_is_dated_on_its_award_date_not_the_broker_s_report_date` | **Reading B of Q17: Zufluss on the booking day, value of that day.** Where the settled law ends is stated at the claim: BMF 01.06.2024 Rn. 25-26 and the BFH line fix the rule on both sides, and no German source at any tier says on which side this programme's *void* clause falls -- that turns on the effect of the clause under the foreign law governing the holding, a fact nobody has established. **Decided by the maintainer (uebber), 2026-09-20:** the effect of one customer's contract is a matter of that individual legal relationship, not something this engine can find; Reading B is taken, and recorded as **defensible (vertretbar) and not negligent**. Why it is defensible: booking is the Tier 2 default (Rn. 25) and deferral the exception; the four clause-tied grounds at the claim (dividends and vote from the award; price risk and margin use with the recipient; a bilateral contractual Sperrfrist over ordinary listed shares, unlike the administration's own example of a restriction carried by the share; a resolutive reclaim) each rest on Tier 2 or the BFH line; and it contradicts no Tier 1 or Tier 2 source. Why it is not negligent: the question was researched, both readings and their authority are written into the store as Q17, and the choice is stated here and in the run's output instead of being made in silence. The contributor, whose declaration it is, agreed the same reading on 2026-09-20. **What Reading A would change**, so the choice can be reversed if the fact is ever established: Zufluss moves to `VestingDate`, the lot takes the vesting row's `Price` at that day's ECB rate, and a return before vesting has no tax effect; the engine does not implement that path, and adopting it would be a `fix-func(engine)`. **Correction 2026-09-20, kept:** this row once said Leitsatz 2 of VI R 37/09 *does not arise, on the law*, because the restriction is contractual; VI R 37/09 Rn. 20 makes the effect of a restriction decisive and its source irrelevant, so that sentence was wrong. **Input contract:** acquisition date ← `AwardDate`; unit value ← the award row's `Price` in `CurrencyPrimary`; EUR ← ECB rate of `AwardDate`. An unparseable date, a zero quantity, an award `Price` that is not positive, or a failed conversion stops the run (the first three collected in `create_events_from_grants`; `add_lot_for_stock_award` refuses a lot without a EUR cost). The price guard exists because a listed share has a market price, so a zero is a missing valuation: read as a value it gave the lot a nil basis and declared the whole later proceeds as gain (cold review 2026-09-20; `tests/test_stock_award_scenarios.py::test_an_award_without_a_positive_value_stops_the_run`). **Residual, not resolvable from the files:** no column says which day `Price` was struck on; on every award row of the contributor's files `ReportDate` equals `AwardDate`, so the question does not arise there (measured 2026-09-20 by comparing the two columns row by row). |
| GT-ESTG20-065 | **implements** | `src/engine/fifo_manager.py` — the awarded lot's `unit_cost_basis_eur` is the award-day value and is what a later disposal is measured against | `tests/test_stock_award_lots.py::test_the_whole_sequence_leaves_the_broker_s_quantity_and_the_awarded_cost`; `tests/test_stock_award_scenarios.py::test_a_sub_freigrenze_award_still_carries_full_basis` | The value at Zufluss is the Anschaffungskosten: acquisition for consideration (§ 20 Abs. 4 Satz 1, § 255 Abs. 1 Satz 1 HGB), confirmed without regard to taxation of the receipt by BMF 06.03.2025 Rn. 75 and BMF 14.05.2025 Rz. 87 — both analogies, labelled so in the store. **Q16 is retired; this is no longer a filing position.** The earlier *choice under uncertainty* wording and the *what was taxed becomes the basis* rationale are withdrawn. The amount depends on the Zufluss day, so it follows the reading taken at GT-ESTG20-064 (Q17, Reading B, maintainer's decision of 2026-09-20). |
