...
| GT-ESTG20-068 | **deviates** | `src/processing/enrichment.py` adds the broker commission to a buy's EUR cost basis, so that Nebenkosten is capitalised; a **transaction tax on the purchase is not read at all** — `RawTradeRecord` has no field for it and `TRADES_COLUMNS` refuses an export that carries the column | — (none yet) | Closed by the `fix-func` on the trade transaction tax, which this row travels with: the tax joins the cost basis of the lot it was charged on. Until then a lot bought with a purchase tax is carried too low and the gain on its disposal is overstated by the tax. The foreign currency the tax consumed follows the engine's existing choice for the currency leg of a purchase (GT-FX-007, Q9) and is not decided by this claim. This claim is also the first citation for the commission capitalisation, which had no row. |
