...
- **Decimal Parsing:** Numerical monetary values and quantities are parsed into Python's `Decimal` type, preserving precision from the string representation. A blank **optional** decimal is `None` — absent, not zero. A blank **required** decimal fails validation, and by the row rule below that stops the run. So does a cell that is present and **is not a number** — until September 2026 the six older models read one as `Decimal("0.0")`. That includes `NaN` and `Infinity`, which parse as decimals and are no figure, and **any cell containing a comma**: `safe_decimal` would guess `"1,234"` to be 1.234, and a guess that can be a thousand times off is not a reading. Measured over every export in `data_import/`: 0 of 2,185 non-blank decimal cells are unparsable, and none contains a comma. There is no `Decimal("1.0")` default; the description that claimed one predates issue #47.
...
| `IBCommission`         | `ib_commission`             | `Optional[Decimal]`         | What the trade cost in commission, **net**: the broker's own charge and the venue fees and rebates it passes on, as one signed figure. A charge is negative, a credit positive. | Optional. Usually negative. Example: "-11.0", "-1.00000". **Positive on a few rows** — a venue's rebate for the order larger than the broker's charge (the broker's pricing pages describe its tiered plan as "commission plus external venue fees/rebates"). The export never splits the figure into fee and rebate. A charge raises a buy's cost and lowers a sale's proceeds [GT-ESTG20-068]; a credit does the opposite [GT-ESTG20-069; Q21's execution-price distinction]. Until September 2026 the sign was discarded and a credit was booked as a charge. |
...
| `Taxes`                | `taxes`                     | `Decimal`                   | Transaction tax the broker charged on the trade itself (a stamp duty or financial transaction tax of the market traded on). Denominated in `CurrencyPrimary`; the export has no currency column for it. A charge is negative. On a purchase it is a Nebenkosten of the acquisition and joins the cost basis of the lot bought; on a sale it is a cost of the disposal and lowers the proceeds [GT-ESTG20-068]. Either way it is part of the cash the trade moved: a buy pays gross + tax, a sale receives gross − tax. | **Required column and required value** — an export without the column stops the run, as does a blank cell, rather than reading either as "no tax". "0" on almost every row. Measured on `Trades-2022` to `-2025`: 236 rows, blank on 0, non-zero on 2 — both negative, both `STK` `BUY` with `Open/CloseIndicator` `O`, both with `IBCommissionCurrency` equal to `CurrencyPrimary`. None of the 383 cash-transaction rows of the same window repeats either charge, so the tax reaches the engine through this column only. **Currency and sign are confirmed by the broker's own cash balance, not assumed:** before the column was read, the engine's closing balance in each of the two currencies differed from `Cash_Balance` `EndingCash` by exactly that row's `Taxes` value; read as a charge in `CurrencyPrimary`, both balances agree (`VALIDATION_REPORT.md`, 2026-09-21). A charge on a **sale** has not occurred yet either; it is treated, because the law is as clear for it as for a purchase and the column's sign convention is the observed one (negative = charge). **Three shapes have no treatment and stop the run, naming every such row:** a positive value (a credit); a non-zero value on any row that is not `AssetClass` `STK` — options, futures, bonds, currency pairs; and a non-zero value on a trade whose value is zero (`Quantity` × `TradePrice`), where the tax would reach the cost side while the currency side, which is drawn with the trade's value, drew nothing. |

**Maintainer input check, 2026-09-22 (separate from the contributor's counts above):**
the refreshed 2021–2025 exports contain 6,976 Trades rows, all with nonblank Taxes.
Nonzero tax counts by year are 0/8/1/7/30. All are negative charges on STK;
none of the 6,976 rows is a taxed sale with tax at least equal to gross proceeds.
Positive commissions occur on 166 rows (110 OPT, 56 STK), so stock-only exposure
and unchanged early-year results must not be inferred from the contributor's data.
The sale-tax boundary is exercised synthetically. The real VZ 2023 short-sale
refusal is instead an Ex stock delivery whose raw net is positive before the
existing option-premium adjustment; signed lot support does not settle that
separate option-treatment deviation.
...
| `TransactionID`    | `transaction_id`            | `Optional[str]`     | Summary-row id; blank on lot rows.                       | Carried onto a cash move's event (it orders the move intra-day); not onto a securities move's (see below).    |
...
**Notes:**
- **`TransferPrice` and `CostBasis` are parsed but never consumed.** They *are* a cost basis, but in IBKR's convention: an option-assignment premium is netted into the basis of the assigned shares, which German law rejects ([GT-ESTG20-004], BMF 14.05.2025 Rz. 26). The German-correct basis is the sending account's own reconstruction (with separate option-treatment limitations recorded in the legal implementation map), so the lots relocate with *that* basis and these two values are read to satisfy the required-column contract, not to compute or check a figure. What the `LOT` rows are used for -- the acquisition day, the quantity, and the long/short sign -- is matched and cross-checked against the sending ledger.
- **`TransactionID` is carried onto a cash move's event, not a securities move's.** For a securities move the two sides carry different ids, so neither names the move and none is put on the event; the move sorts before that day's trades by rule, so units that arrived can be spent. For a cash move the two sides carry the **same** id, which pairs them into one move (and keeps two genuinely distinct same-shape moves apart) and is carried onto the event: a currency Umbuchung sits in the trade band ordered by its id, so the broker's own intra-day chronology decides where it lands among the day's currency events. That order is load-bearing — the currency FIFO gain is right only if the lots are consumed in the broker's true order ([GT-FX-010]).
...
- **The price's own date is not stated.** § 8 Abs. 2 Satz 1 wants the übliche Endpreis on the day of
  ...
  be either day's. The importer treats the award row's price as the value assigned to that
  award. The contributor's recorded measurement found equal dates on every award row
  (2026-09-20); the maintainer has no Grants files. No claim of independent broker-field
  authentication is made. A differing-date award needs its valuation semantics established
  before relying on its figures. Vesting-row prices do not supply receipt or basis.
