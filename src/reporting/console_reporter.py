...
def generate_console_tax_report(
    ...
    vorabpauschale_gaps: Optional[List] = None,
):
    ...
    relevant_gaps = [g for g in (vorabpauschale_gaps or []) if getattr(g, "deemed_inflow_year", None) == tax_year]
    if relevant_gaps:
        bar = "=" * 70
        print(f"  {bar}")
        ...
        for g in relevant_gaps:
            print(f"    - {g.description}: {g.reason}")
