...
The conditions routed through this channel, as of 2026-09-19:
    ...
    FOREIGN_WHT_CONDITIONS_YEAR_NOT_RESEARCHED         WARNING
    FOREIGN_WHT_FACTS_UNANSWERED                       WARNING
    FOREIGN_WHT_CONDITION_UNRESOLVED                   WARNING
...
The foreign-withholding codes do not follow that shape; they are WARNINGs by the
maintainer's decision on PR #102 (second review): an unresolved credit may be left
unclaimed, never claimed under a warning. `FOREIGN_WHT_RATE_NOT_VERIFIED`,
`FOREIGN_WHT_RATE_YEAR_NOT_RESEARCHED`, `FOREIGN_WHT_UNLINKED`,
`FOREIGN_WHT_CONDITIONS_YEAR_NOT_RESEARCHED`, `FOREIGN_WHT_FACTS_UNANSWERED` and
`FOREIGN_WHT_CONDITION_UNRESOLVED` list the rows with no supported creditable amount.
Those rows are not on Zeile 41, so the declared figure claims no unsupported credit;
the report lists each with what is open. `FOREIGN_WHT_ABOVE_TREATY_RATE`: the row is
