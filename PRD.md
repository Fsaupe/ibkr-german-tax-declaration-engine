...
**SOY Quantity:** Each account's SOY quantity (`Decimal`) for an `Asset` (as established per Section 2.1, i.e., primarily from positions start file, or `Decimal(0)` if not listed for non-cash assets) is the definitive starting quantity for its FIFO ledger at the beginning of the tax year. This applies to both long (positive) and short (negative) positions.
...
- **Bond Maturity (BM):** Redemption of a bond at maturity (Fälligkeit). Under §20 Abs. 2 Satz 2 EStG the Einlösung is deemed a Veräußerung, with the gain falling under §20 Abs. 2 Satz 1 Nr. 7. There is no IBKR trade record for the maturity itself — only a corporate action record with `Type="BM"`. The parser maps it to a synthetic `TradeEvent` of type `TRADE_SELL_LONG` (proceeds = the CA record's total `Proceeds`, price per bond = proceeds / quantity, no commission), so it reuses the bond FIFO and currency-impact path and is replayed historically like any other trade. Realized G/L (`RealizationType.LONG_POSITION_SALE`) is reported in Anlage KAP as Other Capital Income: positive → Zeile 19 (`kap_other_income_positive`), negative → Zeile 22 (`kap_other_losses_abs`), consistent with bond sales. **Input validation (fail-fast):** the CA record's `Quantity` must be negative (bonds removed) and `Proceeds` must be strictly positive; either violation raises `DataIntegrityError`. A blank `Proceeds` cell parses to `Decimal("0")`, and accepting it would book a deductible loss equal to the entire cost basis — under §20 Abs. 4 EStG an unknown Veräußerungserlös makes the gain incomputable, so it is rejected rather than inferred. Consequences: a bond held **short** to maturity (positive quantity) and a genuine redemption at zero (issuer default, which IBKR reports as a write-off rather than `Type="BM"`) are both unsupported and fail loudly.
